...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // ------------------------------------------
    // 1. INJECT HEADER & FOOTER (One Code Rule)
    // ------------------------------------------
    const headerHTML = `
    <header class="header">
    ...
    // ------------------------------------------
    // 3. ACTIVE NAVIGATION LOGIC (Must run AFTER injection)
    // ------------------------------------------
    const currentPage = window.location.pathname.split("/").pop();
    const navLinks = document.querySelectorAll('.nav-link');
    
    navLinks.forEach(link => {
        // Check if href matches current page OR if we are on root and link is index.html
        if(link.getAttribute('href') === currentPage || (currentPage === '' && link.getAttribute('href') === 'index.html')) {
            link.classList.add('active');
        }
    });

    // ... REST OF YOUR EXISTING CODE (Product Catalog, Modals, Forms, etc.) ...
    ...
    if (productsGrid) {
        ...
        productCatalog.forEach(categoryData => {
            ...
            // NOTE: Using innerHTML for structure, data is trusted (internal)
            card.innerHTML = `
                ...
                <div class="sub-products-list">
                    ${categoryData.subProducts.map(sub => `
                        <div class="sub-item" 
                            data-product-name="${sub.name}" 
                            data-specs="${sub.specs}" 
                            data-ingredients="${sub.ingredients}" 
                            data-uses="${sub.uses.join('|')}" 
                            data-image="${sub.image}"
                            data-hscode="${sub.hsCode}" 
                            data-dgft="${sub.dgftDesc}">
    ...
    // Cascading Dropdown & Auto-fill HS Code
    if (productReqSelect && productTypeSelect) {
        ...
        // When Variety is selected, fill HS Code fields
    ...
    if (exactReqTextarea && wordLimitCounter) {
        ...
        exactReqTextarea.addEventListener('input', () => {
            ...
            if (words.length > maxWords) {
                // Truncate logic is complex to do perfectly while typing, 
                // so we just warn visually and stop form submit (see below)
    ...
    if (quotationForm) {
        quotationForm.addEventListener('submit', function(e) {
            ...
            emailjs.send(serviceID, templateID, {
                ...
                hs_code: data.hs_code, // Auto-filled value
                dgft_description: data.dgft_description, // Auto-filled value
    ...
    if (closePopup) {
        closePopup.addEventListener('click', () => popupMessage.style.display = "none");
    }

    // ------------------------------------------
    // 6. PARTNER & CERTIFICATE MODAL
    // ------------------------------------------
    ...
    // Global Click to Close Modals (Outside click)
    window.onclick = function(event) {
        if (pModal && event.target == pModal) window.closePartnerModal();
        // Terms modal if exists
        const termsModal = document.getElementById('terms-modal');
        if (termsModal && event.target == termsModal) termsModal.style.display = 'none';
    };
});

// ------------------------------------------
// 7. ANTI-THEFT & SECURITY (Disable F12/RightClick)
// ------------------------------------------
